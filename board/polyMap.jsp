...
<style>
.map_wrap, .map_wrap * {margin:0;padding:0;font-family:'Malgun Gothic',dotum,'돋움',sans-serif;font-size:16px;}
.map_wrap a, .map_wrap a:hover, .map_wrap a:active{color:#000;text-decoration: none;}
.btn_map_area input{margin-bottom:5px}
.map_wrap {position:relative;width:830px;height:330px;}
#menu_wrap {position:absolute;top:330px;left:0;bottom:0;width:830px;height:380px;margin:10px 0 30px 10px;padding:5px;overflow-y:auto;background:rgba(255, 255, 255, 0.7);z-index: 1;font-size:12px;border-radius: 10px;}
...
#search_map_btn input{width:59px;cursor:pointer;}
#image-map area{cursor:pointer;}
</style>
</head>
<body onload="searchMyPlaces()">
<nav class = "navbar navbar-expand  navbar-dark bg-dark">
	...
	<!-- ===========================맵 표시하는 부분========================= -->
	<div style="margin:30px 0 10px 100px; height:780px">
		...
		<!-- 지역 버튼  -->
		<div style="position: absolute; top: 740px; left:50%; transform: translateX(-540px); width: 320px" class="btn_map_area">
			<input class="btn btn-success" style="width:100%;" type="button" value="내 주변 PolyChicken 찾기" onclick="searchMyPlaces()">
		</div>
		<div id="search_map_btn" style="position: absolute; top: 780px; left:50%; transform: translateX(-540px); width: 320px" class="btn_map_area">
	...
	if (navigator.geolocation) {
	    ...
	    navigator.geolocation.getCurrentPosition(function(position) {
	        ...
	        var lat = position.coords.latitude, // 위도
	            ...
	            //키워드 검색
	       		ps.keywordSearch( "bhc", placesSearchCB,{x: lon, y: lat});
	        	// 마커와 인포윈도우를 표시합니다
	        	displayMarker(locPosition,lat,lon);
	       		document.getElementById('keyword').value = "내 주변 BHC";
	      });
	    
	} else { // HTML5의 GeoLocation을 사용할 수 없을때 마커 표시 위치와 인포윈도우 내용을 설정합니다
	    alert('geolocation을 사용할수 없어요..');
	    ps.keywordSearch("서울 BHC", placesSearchCB);
	}
}
//지도에 마커와 인포윈도우를 표시하는 함수입니다
function displayMarker(locPosition,px,py) {
